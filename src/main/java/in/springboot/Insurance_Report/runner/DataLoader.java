package in.springboot.Insurance_Report.runner;

import in.springboot.Insurance_Report.entity.CitizenPlan;
import in.springboot.Insurance_Report.repo.CitizenPlanRepository;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.boot.ApplicationArguments;
import org.springframework.boot.ApplicationRunner;
import org.springframework.stereotype.Component;

import java.time.LocalDate;
import java.util.ArrayList;
import java.util.Arrays;
import java.util.List;

@Component
public class DataLoader implements ApplicationRunner {
    @Autowired
    private CitizenPlanRepository repo;


    @Override
    public void run(ApplicationArguments args) throws Exception {

        repo.deleteAll();

        CitizenPlan c1 = new CitizenPlan();
        c1.setCitizenName("John");
        c1.setGender("Male");
        c1.setPlanName("Cash");
        c1.setPlanStatus("Approved");
        c1.setPlanStartDate(LocalDate.now());
        c1.setPlanEndDate(LocalDate.now().plusMonths(6));
        c1.setBenefitAmt(5000);

        CitizenPlan c2 = new CitizenPlan();
        c2.setCitizenName("Smith");
        c2.setGender("Male");
        c2.setPlanName("Cash");
        c2.setPlanStatus("Denied");
        c2.setDenialReason("High Income");

        CitizenPlan c3 = new CitizenPlan();
        c3.setCitizenName("Karry");
        c3.setGender("FeMale");
        c3.setPlanName("Cash");
        c3.setPlanStatus("Terminated");
        c3.setPlanStartDate(LocalDate.now().minusMonths(4));
        c3.setPlanEndDate(LocalDate.now().plusMonths(2));
        c3.setTerminationDate(LocalDate.now());
        c3.setBenefitAmt(5000);
        c3.setTerminationRsn("Other Income");

        //Food Plan
        CitizenPlan c4 = new CitizenPlan();
        c4.setCitizenName("Jack");
        c4.setGender("Male");
        c4.setPlanName("Food");
        c4.setPlanStatus("Approved");
        c4.setPlanStartDate(LocalDate.now());
        c4.setPlanEndDate(LocalDate.now().plusMonths(6));
        c4.setBenefitAmt(5000);

        CitizenPlan c5 = new CitizenPlan();
        c5.setCitizenName("Jacob");
        c5.setGender("Male");
        c5.setPlanName("Food");
        c5.setPlanStatus("Denied");
        c5.setDenialReason("High Income");

        CitizenPlan c6 = new CitizenPlan();
        c6.setCitizenName("Carolina");
        c6.setGender("FeMale");
        c6.setPlanName("Food");
        c6.setPlanStatus("Terminated");
        c6.setPlanStartDate(LocalDate.now().minusMonths(4));
        c6.setPlanEndDate(LocalDate.now().plusMonths(2));
        c6.setTerminationDate(LocalDate.now());
        c6.setTerminationRsn("Farm Income");
        c6.setBenefitAmt(5000);

        //Medical Plan
        CitizenPlan c7 = new CitizenPlan();
        c7.setCitizenName("Mark");
        c7.setGender("Male");
        c7.setPlanName("Medical");
        c7.setPlanStatus("Approved");
        c7.setPlanStartDate(LocalDate.now());
        c7.setPlanEndDate(LocalDate.now().plusMonths(6));
        c7.setBenefitAmt(5000);

        CitizenPlan c8 = new CitizenPlan();
        c8.setCitizenName("Neil");
        c8.setGender("Male");
        c8.setPlanName("Medical");
        c8.setPlanStatus("Denied");
        c8.setDenialReason("High Income");

        CitizenPlan c9 = new CitizenPlan();
        c9.setCitizenName("Markiee");
        c9.setGender("FeMale");
        c9.setPlanName("Medical");
        c9.setPlanStatus("Terminated");
        c9.setPlanStartDate(LocalDate.now().minusMonths(4));
        c9.setPlanEndDate(LocalDate.now().plusMonths(2));
        c9.setTerminationDate(LocalDate.now());
        c9.setTerminationRsn("REnt Income");
        c9.setBenefitAmt(5000);

        //Employment Plan
        CitizenPlan c10 = new CitizenPlan();
        c10.setCitizenName("Charles");
        c10.setGender("Male");
        c10.setPlanName("Employment");
        c10.setPlanStatus("Approved");
        c10.setPlanStartDate(LocalDate.now());
        c10.setPlanEndDate(LocalDate.now().plusMonths(6));
        c10.setBenefitAmt(5000);

        CitizenPlan c11 = new CitizenPlan();
        c11.setCitizenName("Xian");
        c11.setGender("Male");
        c11.setPlanName("Employment");
        c11.setPlanStatus("Denied");
        c11.setDenialReason("Property Income");

        CitizenPlan c12 = new CitizenPlan();
        c12.setCitizenName("Katherine");
        c12.setGender("FeMale");
        c12.setPlanName("Employment");
        c12.setPlanStatus("Terminated");
        c12.setPlanStartDate(LocalDate.now().minusMonths(4));
        c12.setPlanEndDate(LocalDate.now().plusMonths(2));
        c12.setTerminationDate(LocalDate.now());
        c12.setTerminationRsn("Govt Income");
        c12.setBenefitAmt(5000);


        List<CitizenPlan> list= Arrays.asList(c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11, c12);

        repo.saveAll(list);
    }
}
